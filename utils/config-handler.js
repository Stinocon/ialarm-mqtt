...
function initDefaults (config, configFile) {
  ...
  if (config) {
    ...
    // diagnostics are published on their own (slower) timer: they change on every
    // poll and would otherwise flood the broker and the HA recorder
    _checkConfig(config, ['server', 'polling_diagnostics'], 0, 60000)
    _checkConfig(config, ['server', 'features'], 0, ['armDisarm', 'sensors', 'events', 'bypass', 'zoneNames', 'zoneId', 'diagnostics'])
    ...
    _checkConfig(config, ['topics', 'availability'], 0, topicPrefix + '/alarm/availability')
    _checkConfig(config, ['topics', 'alarm'], 0, {})
    _checkConfig(config, ['topics', 'alarm', 'command'], 0, topicPrefix + '/alarm/area/${areaId}/set')
    _checkConfig(config, ['topics', 'alarm', 'state'], 0, topicPrefix + '/alarm/state')
    _checkConfig(config, ['topics', 'alarm', 'event'], 0, topicPrefix + '/alarm/event')
    _checkConfig(config, ['topics', 'alarm', 'bypass'], 0, topicPrefix + '/alarm/zone/${zoneId}/bypass')
    _checkConfig(config, ['topics', 'alarm', 'discovery'], 0, topicPrefix + '/alarm/discovery')
    _checkConfig(config, ['topics', 'alarm', 'resetCache'], 0, topicPrefix + '/alarm/resetCache')
    _checkConfig(config, ['topics', 'alarm', 'configStatus'], 0, topicPrefix + '/alarm/configStatus')
    _checkConfig(config, ['topics', 'sensors'], 0, {})
    _checkConfig(config, ['topics', 'sensors', 'topicType'], 0, 'state')
    _checkConfig(config, ['topics', 'sensors', 'state'], 0, topicPrefix + '/sensors/state')
    _checkConfig(config, ['topics', 'sensors', 'zone'], 0, {})
    _checkConfig(config, ['topics', 'sensors', 'zone', 'state'], 0, topicPrefix + '/sensors/zone_${zoneId}')
    _checkConfig(config, ['topics', 'sensors', 'zone', 'alarm'], 0, topicPrefix + '/sensors/zone_${zoneId}/alarm')
    _checkConfig(config, ['topics', 'sensors', 'zone', 'active'], 0, topicPrefix + '/sensors/zone_${zoneId}/wirelessLoss')
    _checkConfig(config, ['topics', 'sensors', 'zone', 'lowBattery'], 0, topicPrefix + '/sensors/zone_${zoneId}/lowbat')
    _checkConfig(config, ['topics', 'sensors', 'zone', 'fault'], 0, topicPrefix + '/sensors/zone_${zoneId}/fault')
    _checkConfig(config, ['topics', 'zonesDirectory'], 0, topicPrefix + '/zones/directory')
    _checkConfig(config, ['topics', 'diagnostics'], 0, topicPrefix + '/alarm/diagnostics')
    ...
    _checkConfig(config, ['hadiscovery', 'topics', 'alarmConfig'], 0, '${discoveryPrefix}/alarm_control_panel/' + discoveryDeviceKey + '_${areaId}/config')
    _checkConfig(config, ['hadiscovery', 'topics', 'eventsConfig'], 0, '${discoveryPrefix}/sensor/' + discoveryDeviceKey + '/events/config')
    _checkConfig(config, ['hadiscovery', 'topics', 'connectionConfig'], 0, '${discoveryPrefix}/binary_sensor/' + discoveryDeviceKey + '/connection/config')
    _checkConfig(config, ['hadiscovery', 'topics', 'sensorConfig'], 0, '${discoveryPrefix}/binary_sensor/' + discoveryDeviceKey + '_zone_${zoneId}/fault/config')
    _checkConfig(config, ['hadiscovery', 'topics', 'sensorBatteryConfig'], 0, '${discoveryPrefix}/binary_sensor/' + discoveryDeviceKey + '_zone_${zoneId}/battery/config')
    _checkConfig(config, ['hadiscovery', 'topics', 'sensorAlarmConfig'], 0, '${discoveryPrefix}/binary_sensor/' + discoveryDeviceKey + '_zone_${zoneId}/alarm/config')
    _checkConfig(config, ['hadiscovery', 'topics', 'sensorConnectivityConfig'], 0, '${discoveryPrefix}/binary_sensor/' + discoveryDeviceKey + '_zone_${zoneId}/connectivity/config')
    _checkConfig(config, ['hadiscovery', 'topics', 'sensorZoneIdConfig'], 0, '${discoveryPrefix}/sensor/' + discoveryDeviceKey + '_zone_${zoneId}/id/config')
    _checkConfig(config, ['hadiscovery', 'topics', 'zoneDirectoryConfig'], 0, '${discoveryPrefix}/sensor/' + discoveryDeviceKey + '/zone_directory/config')
    _checkConfig(config, ['hadiscovery', 'topics', 'diagnosticsConfig'], 0, '${discoveryPrefix}/sensor/' + discoveryDeviceKey + '/diagnostics/config')
    _checkConfig(config, ['hadiscovery', 'topics', 'lastPollConfig'], 0, '${discoveryPrefix}/sensor/' + discoveryDeviceKey + '/last_poll/config')
    _checkConfig(config, ['hadiscovery', 'topics', 'connectionErrorsConfig'], 0, '${discoveryPrefix}/sensor/' + discoveryDeviceKey + '/connection_errors/config')
    _checkConfig(config, ['hadiscovery', 'topics', 'panelDisconnectionsConfig'], 0, '${discoveryPrefix}/sensor/' + discoveryDeviceKey + '/panel_disconnections/config')
    _checkConfig(config, ['hadiscovery', 'topics', 'bypassConfig'], 0, '${discoveryPrefix}/switch/' + discoveryDeviceKey + '/${zoneId}/config')
    _checkConfig(config, ['hadiscovery', 'topics', 'clearCacheConfig'], 0, '${discoveryPrefix}/switch/' + discoveryDeviceKey + '/clear_cache/config')
    _checkConfig(config, ['hadiscovery', 'topics', 'clearDiscoveryConfig'], 0, '${discoveryPrefix}/switch/' + discoveryDeviceKey + '/clear_discovery/config')
    _checkConfig(config, ['hadiscovery', 'topics', 'clearTriggeredConfig'], 0, '${discoveryPrefix}/switch/' + discoveryDeviceKey + '/clear_triggered/config')
